...
<!-- IE7 fixes made by Keith Brunton, School of Physics, Edinburgh University - 02/02/07 -->
...
function MPInstall(){
 <!-- ie7 issues mean that the complex MathPlayer install dialog fails to display properly -->
 <!-- For ie7 a simple alert is shown asking users to install MathPlayer and reload. -->
 var ie7 = (document.all &amp;&amp; !window.opera &amp;&amp; window.XMLHttpRequest);
 if (ie7) {
   var exp = new Date();
   var thirtyDays = exp.getTime() + (30 * 24 * 60 * 60 * 1000);
   exp.setTime(thirtyDays);
   alert("This page requires Design Science's MathPlayer&#8482; to display Mathematics. \nPlease download and install MathPlayer from \n\nhttp://www.dessci.com/webmath/mathplayer\n\nReload this page once MathPlayer is installed.");
   document.cookie = "MathPlayerInstall=0;expires="+exp.toGMTString();
   return 0;
 }
 else {
   var showDialog=true;
...
function MPDialog() {
 ...
 text += 'document.cookie="MathPlayerInstall="+v+cookieProps;'
 text += 'window.close();'
 text += '}'
 text += '&lt;/' + 'script>'
...
<!-- TODO: If pmathmlcss.xsl is not available then the following code may result in a script error -->
