...
\begin{enumerate}
  \item \href[\textbf{DOM Outputs}]{#dom}: These create standalone branches within a DOM.
    Use these if you just want to get an XML ``fragment'' or want to import
    SnuggleTeX output into an existing DOM Docment.

  \item \href[\textbf{Web Page Outputs}]{#web}: These generate full DOM Documents and/or
    web pages, providing programmatic control over how the process
    works.
\end{enumerate}

\subsection*{\anchor{dom}DOM Outputs}
...
\subsection*{\anchor{web}Web Page Outputs}
...
\subsection*{\anchor{mathml}XHTML + MathML Web Pages}
...
\begin{itemize}
  ...
  \item \anchor{xsl} \textbf{CLIENT\_SIDE\_XSLT\_STYLESHEET}: This generates a XHTML + MathML document,
...
\subsection*{\anchor{legacy}Legacy (X)HTML + Images Web Pages}

You will need to ensure you have downloaded the full binary
distribution of SnuggleTeX for this, and add all of the JARs under the
\verb|lib/jeuclid| folder
...
This feature is still experimental\ldots though the \href[pages you are now
reading]{latex-samples.html} are generated using it!
